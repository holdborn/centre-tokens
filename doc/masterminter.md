...
`masterMinter` role in the CENTRE TZSC contract to multiple addresses. (The
...
- `minterManager` - address of a contract (e.g. TZSC) with a
...
`MinterManagementInterface`. The `MasterMinter` interacts with the TZSC token
