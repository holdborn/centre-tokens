...
contract("V2_1Upgrader", (accounts) => {
  ...
  before(async () => {
    ...
    await fiatTokenProxy.upgradeToAndCall(
      ...
      web3.eth.abi.encodeFunctionSignature("initializeV2(string)") +
        strip0x(web3.eth.abi.encodeParameters(["string"], ["TZS Coin"])),
  ...
  describe("upgrade", () => {
    it("upgrades, transfers proxy admin role to newProxyAdmin, runs tests, and self-destructs", async () => {
      ...
      // Transfer 0.2 TZSC to the upgrader contract
      await proxyAsV2.transfer(upgrader.address, 2e5, { from: minter });

      // Transfer 100 TZSC to the FiatTokenProxy contract
      ...
      // 0.2 TZSC is transferred back to the upgraderOwner
      expect(
      ...
      // the TZSC tokens held by the proxy contract are transferred to the lost
      // and found address
      expect(
